...
export default function NotificacoesBell({ barbeariaId }) {
  ...
  const carregarNotificacoesDoDia = async () => {
    ...
    try {
      ...
      if (!error && data) {
        // Filtra apenas os agendamentos correspondentes à data de hoje
  ...
  useEffect(() => {
    ...
    // Atualização silenciosa a cada 15 segundos para refletir novos agendamentos em tempo real
      ...
      {modalNotifAberto && (
        <div className="absolute right-0 mt-3 w-80 sm:w-96 bg-white rounded-[2.5rem] shadow-2xl border border-stone-200/80 p-6 z-50 animate-in fade-in zoom-in-95 duration-150">
          <div className="flex items-center justify-between pb-3.5 border-b border-stone-100">
            <h3 className="font-black text-stone-900 text-sm tracking-tight">Atendimentos de Hoje</h3>
            <button 
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="text-[10px] text-stone-400 block mt-1">
                      Horário: {item.data_hora ? new Date(item.data_hora).toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'}) : '--:--'}
                    </span>
                  </div>
                </div>
              ))
