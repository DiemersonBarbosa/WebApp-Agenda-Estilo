...
export default function NotificacoesBell({ barbeariaId }) {
  ...
  const carregarNotificacoesDoDia = async () => {
    ...
    try {
      let query = supabase
        ...
        `)
        .eq('lido', false) // Traz apenas os que ainda não foram limpidos/lidos online
  ...
  const limparNotificacoesOnline = async () => {
    ...
    try {
      // Atualiza online na base de dados para que fiquem marcados como lidos
      ...
      if (!error) {
        setAgendamentosHoje([]); // Esvazia a lista instantaneamente na tela
      }
    } catch (err) {
