...
export default function AgendamentoChat({ barbeariaId }) {
  ...
  // Fundo exterior totalmente dinâmico que elimina qualquer margem preta indesejada
  ...
  const estiloFundoContainer = isClean 
    ? 'bg-gradient-to-br from-[#ffffff] via-[#f8fafc] to-[#f1f5f9] border-slate-300 text-slate-900 shadow-[0_30px_90px_rgba(0,0,0,0.1)]' 
  ...
  const renderCartaoFidelidade = () => {
    ...
    return (
      <div className={`p-4 rounded-3xl border backdrop-blur-xl space-y-3 shadow-lg ${isClean ? 'bg-white/90 border-slate-200 text-slate-900' : 'bg-black/80 border-white/10 text-white'}`}>
        <div className={`flex items-center justify-between border-b pb-2.5 ${isClean ? 'border-slate-100' : 'border-white/10'}`}>
          ...
          <span className={`text-[10px] font-extrabold px-2.5 py-0.5 rounded-full border ${atingiuMeta ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/40' : isClean ? 'bg-slate-100 text-slate-700 border-slate-200' : 'bg-amber-500/20 text-amber-400 border-amber-500/40'}`}>
            {atingiuMeta ? '🎁 Resgate Disponível!' : `${selos} / ${metaSelos} Selos`}
          </span>
        </div>

        <div className="space-y-1.5">
          <div className={`w-full h-2.5 rounded-full overflow-hidden p-0.5 border ${isClean ? 'bg-slate-100 border-slate-200' : 'bg-slate-950 border-slate-800'}`}>
            <div 
  ...
  return (
    <div className={`min-h-screen w-full flex flex-col items-center justify-center p-4 transition-colors ${estiloFundoGlobal}`}>
      <div className={`w-full max-w-xl mx-auto rounded-[2.5rem] border overflow-hidden relative backdrop-blur-2xl flex flex-col h-[720px] ${estiloFundoContainer}`}>
        ...
        <div className={`shrink-0 z-20 backdrop-blur-xl border-b ${estiloHeader}`}>
          <div className={`relative h-28 w-full overflow-hidden ${isClean ? 'bg-slate-200' : 'bg-stone-900'}`}>
            {barbearia?.capa_url ? (
            ...
            <div className={`absolute inset-0 ${isClean ? 'bg-gradient-to-t from-white/90 via-white/30 to-transparent' : 'bg-gradient-to-t from-black/90 via-black/40 to-transparent'}`}></div>
          </div>

          <div className="px-6 pb-3 pt-0 relative flex items-center gap-3.5 -mt-8">
            <div className={`w-14 h-14 rounded-2xl p-1 shadow-2xl border overflow-hidden flex items-center justify-center shrink-0 backdrop-blur-xl ${isClean ? 'bg-white border-slate-200 shadow-md' : 'bg-black border-white/20'}`}>
              {barbearia?.logo_url ? (
        ...
        <div ref={chatContainerRef} className="flex-1 p-6 space-y-4 overflow-y-auto">
                  ...
                  <button
                    ...
                    className={`py-2.5 rounded-xl text-xs font-black border transition-all backdrop-blur-md ${
                      ocupado
                        ...
                        : isClean ? 'bg-white text-slate-800 border-slate-300 cursor-pointer hover:border-emerald-700' : 'bg-black/50 text-white border-white/10 cursor-pointer hover:border-white/30'
                    }`}
                  >
                    {h}
                  </button>
              ...
              {servicos.map((s) => (
                <button
                  ...
                  className={`p-4 rounded-3xl border text-left flex flex-col justify-between transition-all cursor-pointer backdrop-blur-xl group hover:scale-[1.02] shadow-lg ${
                    isClean ? 'bg-white/80 border-slate-200 hover:border-emerald-700' : 'bg-gradient-to-br from-black/80 via-black/50 to-stone-900/60 border-white/10 hover:border-white/30'
                  }`}
                >
          ...
          {etapa === 'data' && !estaDigitando && (
            <div className={`p-5 rounded-3xl border backdrop-blur-xl space-y-4 shadow-xl ${isClean ? 'bg-white/90 border-slate-200 text-slate-900' : 'bg-gradient-to-br from-black/80 via-black/60 to-stone-900 border-white/15 text-white'}`}>
          ...
          {etapa === 'resumo' && !estaDigitando && (
            <div className={`p-4 rounded-2xl border space-y-2.5 backdrop-blur-md mt-2 ${isClean ? 'bg-white border-emerald-700/40 text-slate-800' : 'bg-black/60 border-emerald-500/40 text-slate-200'}`} style={{ borderColor: `${corTema}66` }}>
      ...
      {/* Rodapé Dinâmico */}
      <div className={`mt-4 text-[10px] font-bold uppercase tracking-widest ${estiloTextoRodape}`}>
