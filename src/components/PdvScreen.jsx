...
export default function PdvScreen({ barbeariaId, supabase, produtos = [], onReload }) {
  ...
  // Filtro de vendas realizadas hoje considerando o dia completo até meia-noite
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 sm:px-0 space-y-6 pb-28 font-sans">
      ...
      {/* HEADER DO PDV */}
      <div 
        className="relative rounded-[2.5rem] p-5 sm:p-8 border border-white/80 overflow-hidden shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4"
        style={{
      ...
      >
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h3 className="text-base sm:text-lg font-extrabold text-stone-900 tracking-tight">PDV / Frente de Caixa</h3>
            <p className="text-xs text-stone-500">Realize vendas de produtos de balcão e gerencie entradas do dia.</p>
          </div>
        </div>
      ...
      {/* GRADE DE PRODUTOS E CARRINHO */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* LISTA DE PRODUTOS DISPONÍVEIS (2 Colunas) */}
        <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-stone-200/85 shadow-[0_10px_30px_rgba(0,0,0,0.03)] p-6 sm:p-8 space-y-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pb-3 border-b border-stone-100">
            <h4 className="font-black text-stone-900 text-sm tracking-wider uppercase">Produtos Disponíveis</h4>
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 bg-stone-50 border border-stone-200 rounded-2xl text-xs font-bold text-stone-900 focus:outline-none focus:border-stone-900"
              />
            </div>
          </div>
              ...
              {produtosFiltrados.map((prod) => {
                ...
                return (
                  <div 
                    ...
                    className={`p-4 rounded-3xl border transition-all flex flex-col justify-between space-y-2.5 ${
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div className="min-w-0 pr-2">
                        <span className="text-[10px] font-extrabold text-stone-400 uppercase tracking-wider block">{prod.categoria || 'Geral'}</span>
                        <h5 className="font-extrabold text-stone-900 text-xs truncate mt-0.5">{prod.nome}</h5>
                      </div>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ${esgotado ? 'bg-rose-100 text-rose-700' : 'bg-emerald-50 text-emerald-700'}`}>
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-stone-200/60">
                      ...
                      {!esgotado && (
                        <span className="text-xs font-bold bg-stone-900 text-white px-3 py-1.5 rounded-xl shadow-xs flex items-center gap-1">
        ...
        {/* CARRINHO / CHECKOUT */}
        <div className="bg-white rounded-[2.5rem] border border-stone-200/85 shadow-[0_10px_30px_rgba(0,0,0,0.03)] p-6 sm:p-8 flex flex-col justify-between space-y-4">
          <div className="space-y-4">
            <div className="pb-3 border-b border-stone-100">
              <h4 className="font-black text-stone-900 text-sm tracking-wider uppercase flex items-center gap-2">
                <ShoppingCart className="w-4 h-4 text-emerald-600" /> Carrinho de Compras
              </h4>
              <p className="text-xs text-stone-400 mt-0.5">Itens selecionados para o balcão.</p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 bg-stone-50 border border-stone-200 rounded-2xl text-xs font-bold text-stone-900 focus:outline-none focus:border-stone-900"
              />
            </div>
            ...
            <div className="space-y-2.5 max-h-52 overflow-y-auto pr-1">
              {carrinho.length === 0 ? (
                <div className="p-8 text-center text-stone-400 text-xs border border-dashed border-stone-200 rounded-2xl">
                  Carrinho vazio. Clique nos produtos ao lado.
                </div>
              ) : (
                carrinho.map((item) => (
                  <div key={item.id} className="p-3 rounded-2xl bg-stone-50 border border-stone-200/70 flex items-center justify-between gap-2">
                    <div className="min-w-0 pr-1">
                      <h6 className="font-bold text-stone-900 text-xs truncate">{item.nome}</h6>
                      <span className="text-[11px] text-stone-500">R$ {Number(item.preco).toFixed(2)} un.</span>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <button onClick={() => removerDoCarrinho(item.id)} className="p-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-xl border border-rose-100">
                  ...
                  return (
                    <button
                      ...
                      className={`p-2.5 rounded-2xl border text-xs font-bold flex flex-col items-center justify-center gap-1 transition-all cursor-pointer ${
                        selecionado ? 'bg-stone-900 text-white border-stone-900 shadow-md' : 'bg-stone-50 text-stone-700 border-stone-200 hover:bg-stone-100'
                      }`}
                    >
          ...
          <div className="space-y-3 pt-4 border-t border-stone-100">
            ...
            <button
              ...
              className={`w-full py-3.5 rounded-2xl text-xs font-bold shadow-md transition-all flex items-center justify-center gap-2 ${
                carrinho.length === 0 || salvando ? 'bg-stone-200 text-stone-400 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer active:scale-95'
              }`}
            >
      ...
      {/* MODAL DE RESUMO DO CAIXA (ATÉ MEIA-NOITE DO DIA ATUAL) */}
      {modalFechamentoAberto && (
        <div className="fixed inset-0 bg-stone-950/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-6 sm:p-8 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-150">
            ...
            {/* CARD DE FATURAMENTO TOTAL DO DIA */}
            <div 
